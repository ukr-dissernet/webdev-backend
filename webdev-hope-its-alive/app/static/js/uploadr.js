...
$(document).ready(function() {
    ...
    initDropbox($("#dropbox_ukr"));
    initDropbox($("#dropbox_source"));
    // Set up the handler for the file input box.
    $("#file-picker_ukr").on("change", function() {
        $("#dropbox_ukr").removeClass('alert-info');
        $("#dropbox_ukr").addClass('alert-success');
        handleFiles(this.files);

    });
    $("#file-picker_source").on("change", function() {
        $("#dropbox_source").removeClass('alert-info');
        $("#dropbox_source").addClass('alert-success');
        handleFiles(this.files);
    });

    // Handle the submit button.
    $("#upload-button").on("click", function(e) {
      if (PENDING_FILES.length == 2)
      {
...
function initDropbox($dropbox) {
    ...
    $dropbox.on("drop", function(e) {
        ...
        $dropbox.text(PENDING_FILES.length + " файлы готовы к загрузке!");
